...
```{r}
fromJSON(txt) # txt = a JSON string, URL or file
```
...
httr::content(x, as = "text", encoding = "UTF-8") # extract content from a request, as = "text" or as ="parsed"


substr(content(r, "text", encoding = "UTF-8"), 1, 1000) 

# content(r, "text", encoding = "UTF-8") = This function extracts the content of the response r as a text string with UTF-8 encoding.

# substr(x, 1, 1000) =  extracts the substring from the first character to the 1000th character of the content.
```

Let's also write it to disk as a JSON file (make sure you have a `data` folder in your working directory):

```{r}
file_con <- file("data/nyt.json") # opens a connection to a file named nyt.json in the data directory. The file function creates a connection object that can be used to write to the file.

writeLines(content(r, "text", encoding = "UTF-8"), con = file_con) #  line writes the content of the HTTP response r to the file
close(file_con) 
