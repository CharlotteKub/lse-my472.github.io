...
UTF-8 (Unicode Transformation Format - 8-bit) is a variable-width character encoding used for electronic communication. It is capable of encoding all 1,112,064 valid character code points in Unicode using one to four one-byte (8-bit) code units.

```{r}
ddir <- "data/" # insert file path to the directory with the data
...
__So when would you ever need to use UTF-16 or UTF-32?__
...
```{bash, eval = FALSE}
cd "/Users/charlottekuberka/Desktop/Political Science/LSE/LSE Courses/LSE MRes Political Science/lse-my472.github.io/week04"
...
str_view_all(sample_text, "Judge")
```
...
- An escape character is a character that invokes an alternative interpretation on subsequent characters in a string.
- It allows you to include special characters in strings that would otherwise be difficult or impossible to include directly.

e.g. `\n` = new line or `\t` = tab
[] = character class which matches any of the characters inside it
...
str_extract(sample_text, "[(]Entered[:] *[0-9/]+[)]") # or using \\s* for space 
```

```{r}
str_extract_all(sample_text, "Judge [A-z]+ [A-Z].? [A-z]+") # [A-z] matches all upperclass AND all lowerclass letters
```
