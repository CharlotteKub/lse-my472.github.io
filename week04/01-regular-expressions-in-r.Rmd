...
```{r}
str_view(sample_text, "@") # using str_view() only highlights the expression matches but still shows all strings

```

```{r}
str_view_all(sample_text, "@") # `str_view_all()` was deprecated in stringr 1.5.0.

```

```{r}
str_view_all(sample_text, "a|e") # will highlight the letter a or e using | separator 
```
...
str_extract_all(sample_text, "a|e")
```
